...
const [candidates, setCandidates] = useState([]);
  ...
  const deleteMultipleCandidates = async (candidateIds) => {
    //alert(typeof(candidateIds));
    //alert(candidateIds);
    let candidateIdsr = { 'ids': candidateIds };
      let candidatesForRemove = JSON.stringify(candidateIdsr);
        //const candidateIdsd = { ids : candidateIds };
    //alert(typeof(candidateIdsd));
    if (window.confirm("Do You Want To Delete Selected Inspectors")) {
      try {
        const res = await fetch("http://localhost:5000/api/candidates/deletemultiplecandidate", {     
          method: 'POST',
