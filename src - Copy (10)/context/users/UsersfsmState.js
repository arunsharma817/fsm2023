...
const UsersfsmState = (props) => {
  ...
  const deleteMultipleUsers = async (userIds) => {
    ...
    if (window.confirm("Do You Want To Delete Selected Clients")) {
        ...
        const res = await fetch("http://localhost:5000/api/users/delmultipleusers", {
          ...
          if (resp) {
            console.log(resp);
            let userArray = [usersForRemove.ids];
            alert("I am here in ClientArray"+userArray)
            {/* I managed "setUsers" from my end to add immediate when successfully added into the database */ }
              {/* setUsers((users) => {      
                   console.log("showing type of "+typeof(clientArray));
             // return users.filter((res) => res._id !== userId);
             return users.filter((value, index) => !clientArray.includes(index));
        })*/ }
          }
        }).catch(error => {
          console.error('There was an error!', error);
  ...
  return (
    <UsersContext.Provider value={{ users, addUser, deleteUser, editUser, deleteMultipleUsers }}>
      {props.children}
