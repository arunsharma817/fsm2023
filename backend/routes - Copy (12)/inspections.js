...
router.post('/create', fetchowner, [
    ...
    try {
        ...
        const inspection = new Inspections({ client_id, product_id, inspector_id });
...
//Route 3 : Update an existing Inspection  using: Put Method "/api/inspections/update". Login Required
...
router.delete('/delete/:id', fetchowner, async (req, res) => {
 ...
 {/*
   ...
   router.get('/getinspection/:id', fetchowner , async (req, res) => {
...
router.delete('/delmultipleinspections', fetchowner, async (req, res) => {
    //console.log("req body type "+typeof(req.body));
    //console.log("req body value "+req.body);
    const {ids}  = req.body;
    console.log("I am here in multiclients"+ids);
      try {
        await Inspections.deleteMany({'_id':{'$in': ids }});
        res.json({ "Success" : "Inspections have been deleted" , inspections : ids }); 
       }  
        catch (error) {
                console.error(error.message);
                res.status(500).send("Internal Server Error ");
