...
const [mainProductsIds, setMainProductsIds] = useState([]);
  ...
  const deleteMultipleMainProducts = async () => {

    let mainProductIdsr = { 'ids': mainProductsIds };
        ...
        const res = await fetch("http://localhost:5000/api/mainproducts/deletemultiplemainproducts", {
          ...
          headers: {
            ...
            'Content-Type': 'application/json'
                  ...
                  const updatedMainProducts= mainProducts.filter((mainProduct) => !mainProductsIds.includes(mainProduct._id));
                  setMainProducts(updatedMainProducts);
                  setMainProductsIds([]);
        }).catch(error => {
          console.error('There was an error!', error);
  ...
  return (
    <mainProductsContext.Provider value={{ mainProducts, mainProductsIds , setMainProductsIds,  setMainProducts,  addMainProducts, deleteMainProducts, editMainProducts, deleteMultipleMainProducts }}>
      {props.children}
