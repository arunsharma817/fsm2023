...
const ImportFamilyMembers = () => {
    ...
    const { setFamilyMembers } = context;
  ...
  const handleImport = () => {
    ...
    if(file) {
      Papa.parse(file, {
        header: true,
        skipEmptyLines: true,
        complete: (newFamilyMembers) => {
  
            // Filter out rows with empty data
            const filteredData = newFamilyMembers.data.filter(
              (item) =>
                Object.values(item).filter((value) => value !== '').length > 0
            );
            axios
        .post('http://localhost:5000/api/familymembers/bulkimport', formData)
        .then((response) => {
          console.log(response.data.message);
          {filteredData.map((importFamilyMember , index) => { 
                      setFamilyMembers((oldFamilyMembers) => {
                        //console.log(results.data);
                        return [...oldFamilyMembers, importFamilyMember];
                      })
                  })}
        })
        .catch((error) => {
  ...
  return (
    <div>
      Import Family Members    <input type="file" onChange={handleFileChange} />
                                <button onClick={handleImport}>Import CSV</button>
    </div>
  )
}

export default ImportFamilyMembers
