...
const [inventorys, setInventorys] = useState([]);
  ...
  const deleteInventory = (getinventoryId) => {
    ...
    const deleteMultipleInventory =  async (inventoryIds) => {
        // Send a DELETE request to the Node.js API to remove the selected tasks from the database
        ...
        if(window.confirm("Do You Want To Delete Selected Inspectors")) {
               ...
               await fetch("http://localhost:5000/api/inventory/deletemultipleinventory", {
                method: 'DELETE',
                headers: {
                    'owner-token': ownerToken,
                    'Accept': 'application/json',
                ...
                .then((data) => {
                    ...
                    const updatedTasks = inventorys.filter(
                    (inventory) => !inventoryIds.includes(inventory._id)
                    );
                    setInventorys(updatedTasks);
