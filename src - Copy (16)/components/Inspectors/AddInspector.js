...
const AddInspector = (props) => {
    ...
    const { addInspector , editInspector  } = context;

    const navigate = useNavigate();
    const params = useParams();
    const inspectorId = props.inspectorId;
    ...
    const handleSubmit = (e) => {
        ...
        if(inspectorId){
            console.log('I am here just before Edit'+inspectorId);
            setFormValues(text);
            const res =   editInspector(text , inspectorId);
            if(res){
                navigate("/inspectors");
            }
            
        }else{
            const res = addInspector(text);
            if(res){
                navigate("/inspectors");
            }
    ...
    useEffect(() => {
      ...
      if(inspectorId){
            {props.inspectors.map((inspector) => {
