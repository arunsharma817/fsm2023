...
const [helpdesks, setHelpdesks] = useState([]);
  ...
  const deleteMultipleHelpdesk = async (helpdeskIds) => {
        ...
        let helpdeskForRemove = JSON.stringify(helpdeskIdsr);

        if(window.confirm("Do you really want to delete the helpdesk items ?")){
          await fetch("http://localhost:5000/api/helpdesk/deletemultiplehelpdesk",{
            method: 'DELETE',
            headers: {
                'owner-token': ownerToken,
                'Accept': 'application/json',
            ...
            body: helpdeskForRemove,
          }).then((response) => response.json())
          .then((data) => {
           
            // Update the state to remove the deleted tasks from the UI
            const updatedTasks = helpdesks.filter(
            (helpdesk) => !helpdeskIds.includes(helpdesk._id)
            );
            setHelpdesks(updatedTasks); 
