...
const ImportContractors = () => {
  ...
  const handleImport = () => {
    ...
    if(file) {
      Papa.parse(file, {
        header: true,
        skipEmptyLines: true,
        complete: (newContractors) => {
  
            // Filter out rows with empty data
            const filteredData = newContractors.data.filter(
              (item) =>
                Object.values(item).filter((value) => value !== '').length > 0
            );
            axios
        .post('http://localhost:5000/api/contractors/bulkimport', formData)
        .then((response) => {
          console.log(response.data.message);
          {filteredData.map((importContractor , index) => { 
                      setContractors((oldContractors) => {
                        //console.log(results.data);
                        return [...oldContractors, importContractor];
                      })
                  })}
        })
        .catch((error) => {
  ...
  return (
    <div>
      Import Contractors    <input type="file" onChange={handleFileChange} />
                                <button onClick={handleImport}>Import CSV</button>
    </div>
  )
}

export default ImportContractors
