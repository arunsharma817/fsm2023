...
const ImportTailors = () => {
    ...
    if(file) {
      Papa.parse(file, {
        ...
        complete: (newTailors) => {
            ...
            const filteredData = newTailors.data.filter(
              (item) =>
                Object.values(item).filter((value) => value !== '').length > 0
            );
            axios
        .post('http://localhost:5000/api/tailors/bulkimport', formData)
        .then((response) => {
          console.log(response.data.message);
          {filteredData.map((importTailor , index) => { 
                      setTailors((oldTailors) => {
                        //console.log(results.data);
                        return [...oldTailors, importTailor];
                      })
                  })}
        })
        .catch((error) => {
  ...
  return (
    <div>
      Import Tailors    <input type="file" onChange={handleFileChange} />
                                <button onClick={handleImport}>Import CSV</button>
    </div>
  )
}

export default ImportTailors
