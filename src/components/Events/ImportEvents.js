...
const ImportEvents = () => {
    ...
    if(file) {
      Papa.parse(file, {
        ...
        complete: (newEvents) => {
            ...
            const filteredData = newEvents.data.filter(
              (item) =>
                Object.values(item).filter((value) => value !== '').length > 0
            );
            axios
        .post('http://localhost:5000/api/events/bulkimport', formData)
        .then((response) => {
          console.log(response.data.message);
          {filteredData.map((importEvent , index) => { 
                      setEvents((oldEvents) => {
                        //console.log(results.data);
                        return [...oldEvents, importEvent];
                      })
                  })}
        })
        .catch((error) => {
  ...
  return (
    <div>
      Import Events    <input type="file" onChange={handleFileChange} />
