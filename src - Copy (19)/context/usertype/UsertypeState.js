...
const [userTypes, setUserTypes] = useState([])
 ...
 const [filteredItems, setFilteredItems] = useState(userTypes);
  ...
  const filterItems = async (text) => {
    ...
    setUserTypes(getUserTypes.data);
    const searchTextLower = text.toLowerCase();
    const filtered = userTypes.filter((userType) =>
    userType.user_type_title.toLowerCase().includes(searchTextLower)
    );
  ...
  const handleSearchTextChange = (e) => {
    ...
    filterItems(e.target.value); // Filter items immediately after updating searchText
  };
  ...
  return (
    <userTypeContext.Provider value={{ filterItems , handleSearchTextChange , searchText, setSearchText , filteredItems, setFilteredItems , userTypes, addUserType, deleteUserType, editUserType, deleteMultipleUserType }}>
      {props.children}
