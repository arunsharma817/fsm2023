...
const [employees, setEmployees] = useState([]);
  ...
  const deleteMultipleEmployees = async (employeeIds) => {
    ...
    if(window.confirm("Do You Want To Delete Selected Employees")) {
      
        await fetch("http://localhost:5000/api/employees/deletemultipleemployee", {     
          method: 'DELETE',
          ...
          const updatedEmployees = employees.filter((employee) => !employeeIds.includes(employee._id));
          setEmployees(updatedEmployees); 
        }).catch((error) => console.error('Error deleting employees:', error));
    }
  }
